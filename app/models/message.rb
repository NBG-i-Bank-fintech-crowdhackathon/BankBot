...
def send_info()
	...
	conn.post do |req|
		req.url '/me/messages?access_token=' + @@fb_token
		req.headers['Content-Type'] = 'application/json'
		req.body = "{ \"recipient\": { \"id\" : \"#{self.user.fb_id}\" }, \"message\": { \"attachment\" : {\"type\":\"template\",\"payload\":{\"template_type\":\"button\",\"text\":\"How can i help you ? \",\"buttons\":[{\"type\":\"postback\",\"title\":\" I would like to check my account balance \",\"payload\":\" I would like to check my account balance \"},{\"type\":\"postback\",\"title\":\"Where is my nearest ATM ?\",\"payload\":\"Where is my nearest ATM ?\"},{\"type\":\"postback\",\"title\":\"I want to check my last transcations \",\"payload\":\"I want to check my last transcations \"}]}} } }"
	end
	conn.post do |req|
		req.url '/me/messages?access_token=' + @@fb_token
		req.headers['Content-Type'] = 'application/json'
		req.body = "{ \"recipient\": { \"id\" : \"#{self.user.fb_id}\" }, \"message\": { \"attachment\" : {\"type\":\"template\",\"payload\":{\"template_type\":\"button\",\"text\":\"How can i help you ? \",\"buttons\":[{\"type\":\"postback\",\"title\":\"I lost my credit card ! \",\"payload\":\"I lost my credit card ! \"},{\"type\":\"postback\",\"title\":\"I want some help \",\"payload\":\"I want some help \"}]}} } }"
	end
end
