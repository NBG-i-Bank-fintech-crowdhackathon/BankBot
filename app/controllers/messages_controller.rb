          ...
          if(message[:message][:text])
            ...
            message[:message][:attachments].each do |attach|
              if attach[:type] == "location" then
                current_user.messages.create(:text => attach[:payload][:coordinates][:lat].to_s + ',' + attach[:payload][:coordinates][:long].to_s , :response => false).handle_location(attach[:payload][:coordinates][:lat] , attach[:payload][:coordinates][:long])
